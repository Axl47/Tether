...
export function normalizeProviderModelOptionsWithCapabilities(
  ...
  switch (provider) {
    ...
    case "gemini":
      return modelOptions as GeminiModelOptions | undefined;
    case "opencode":
...
export function getReasoningEffortOptions(provider: ProviderKind = "codex"): ReadonlyArray<string> {
...
function matchesProviderSlug(provider: ProviderKind, model: string): boolean {
  switch (provider) {
    case "codex":
    ...
    case "cursor":
      return model === "auto" || model.startsWith("composer");
    case "gemini":
...
export function resolveProviderForModel(
  ...
  for (const provider of providers) {
    const normalized = normalizeModelSlug(trimmed, provider);
    if (normalized && matchesProviderSlug(provider, normalized)) {
      return provider;
    }
  }

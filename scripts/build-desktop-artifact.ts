...
interface StagePackageJson {
  ...
  readonly tetherCommitHash: string;
...
const fallbackOptionConfig = <A>(
  primary: Config.Config<Option.Option<A>>,
  fallback: Config.Config<Option.Option<A>>,
): Config.Config<Option.Option<A>> =>
  Config.all({ primary, fallback }).pipe(
    Config.map(({ primary, fallback }) => Option.orElse(primary, () => fallback)),
  );

const fallbackBooleanConfig = (
  primary: string,
  fallback: string,
  defaultValue = false,
): Config.Config<boolean> =>
  Config.all({
    primary: Config.boolean(primary).pipe(Config.option),
    fallback: Config.boolean(fallback).pipe(Config.option),
  }).pipe(
    Config.map(({ primary, fallback }) =>
      Option.getOrElse(primary, () => Option.getOrElse(fallback, () => defaultValue)),
    ),
  );

const BuildEnvConfig = Config.all({
  platform: fallbackOptionConfig(
    Config.schema(BuildPlatform, "TETHER_DESKTOP_PLATFORM").pipe(Config.option),
    Config.schema(BuildPlatform, "T3CODE_DESKTOP_PLATFORM").pipe(Config.option),
  ),
  target: fallbackOptionConfig(
    Config.string("TETHER_DESKTOP_TARGET").pipe(Config.option),
    Config.string("T3CODE_DESKTOP_TARGET").pipe(Config.option),
  ),
  arch: fallbackOptionConfig(
    Config.schema(BuildArch, "TETHER_DESKTOP_ARCH").pipe(Config.option),
    Config.schema(BuildArch, "T3CODE_DESKTOP_ARCH").pipe(Config.option),
  ),
  version: fallbackOptionConfig(
    Config.string("TETHER_DESKTOP_VERSION").pipe(Config.option),
    Config.string("T3CODE_DESKTOP_VERSION").pipe(Config.option),
  ),
  outputDir: fallbackOptionConfig(
    Config.string("TETHER_DESKTOP_OUTPUT_DIR").pipe(Config.option),
    Config.string("T3CODE_DESKTOP_OUTPUT_DIR").pipe(Config.option),
  ),
  skipBuild: fallbackBooleanConfig("TETHER_DESKTOP_SKIP_BUILD", "T3CODE_DESKTOP_SKIP_BUILD"),
...
function resolveGitHubPublishConfig(updateChannel: "latest" | "nightly"):
  ...
  | undefined {
  const rawRepo =
    process.env.TETHER_DESKTOP_UPDATE_REPOSITORY?.trim() ||
...
export function resolveDesktopProductName(version: string): string {
  return resolveDesktopUpdateChannel(version) === "nightly"
    ? "Tether (Nightly)"
    : (desktopPackageJson.productName ?? "Tether (Alpha)");
}
...
const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    appId: "com.axorio.tether",
    artifactName: "Tether-${version}-${arch}.${ext}",
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stageRoot = yield* mkdir({
    prefix: `tether-desktop-${options.platform}-stage-`,
  });
  ...
  const stagePackageJson: StagePackageJson = {
    ...
    tetherCommitHash: commitHash,
    private: true,
    description: "Tether desktop build",
    author: "AxorIO",
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  platform: Flag.choice("platform", BuildPlatform.literals).pipe(
    Flag.withDescription("Build platform (env: TETHER_DESKTOP_PLATFORM)."),
    Flag.optional,
  ),
  target: Flag.string("target").pipe(
    Flag.withDescription(
      "Artifact target, for example dmg/AppImage/nsis (env: TETHER_DESKTOP_TARGET).",
    ),
    Flag.optional,
  ),
  arch: Flag.choice("arch", BuildArch.literals).pipe(
    Flag.withDescription("Build arch, for example arm64/x64/universal (env: TETHER_DESKTOP_ARCH)."),
    Flag.optional,
  ),
  buildVersion: Flag.string("build-version").pipe(
    Flag.withDescription("Artifact version metadata (env: TETHER_DESKTOP_VERSION)."),
    Flag.optional,
  ),
  outputDir: Flag.string("output-dir").pipe(
    Flag.withDescription("Output directory for artifacts (env: TETHER_DESKTOP_OUTPUT_DIR)."),
    Flag.optional,
  ),
  skipBuild: Flag.boolean("skip-build").pipe(
    Flag.withDescription(
      "Skip `bun run build:desktop` and use existing dist artifacts (env: TETHER_DESKTOP_SKIP_BUILD).",
    ),
    Flag.optional,
  ),
  keepStage: Flag.boolean("keep-stage").pipe(
    Flag.withDescription("Keep temporary staging files (env: TETHER_DESKTOP_KEEP_STAGE)."),
    Flag.optional,
  ),
  signed: Flag.boolean("signed").pipe(
    Flag.withDescription(
      "Enable signing/notarization discovery; Windows uses Azure Trusted Signing (env: TETHER_DESKTOP_SIGNED).",
    ),
    Flag.optional,
  ),
  verbose: Flag.boolean("verbose").pipe(
    Flag.withDescription("Stream subprocess stdout (env: TETHER_DESKTOP_VERBOSE)."),
    Flag.optional,
  ),
  mockUpdates: Flag.boolean("mock-updates").pipe(
    Flag.withDescription("Enable mock updates (env: TETHER_DESKTOP_MOCK_UPDATES)."),
    Flag.optional,
  ),
  mockUpdateServerPort: Flag.integer("mock-update-server-port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Mock update server port (env: TETHER_DESKTOP_MOCK_UPDATE_SERVER_PORT)."),
    Flag.optional,
  ),
}).pipe(
  Command.withDescription("Build a desktop artifact for Tether."),

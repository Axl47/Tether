...
it.effect(
  "returns unavailable when codex is below the minimum supported version",
  () =>
    Effect.gen(function* () {
      const status = yield* checkCodexProviderStatus;
      assert.strictEqual(status.provider, "codex");
      ...
      assert.strictEqual(
        status.message,
        "Codex CLI v0.36.0 is too old for Tether. Upgrade to v0.37.0 or newer and restart Tether.",
      );
    }).pipe(
      Effect.provide(
        mockSpawnerLayer((args) => {
...
it.effect(
  "returns unauthenticated when auth probe reports login required",
  () =>
    Effect.gen(function* () {
      const status = yield* checkCodexProviderStatus;
      assert.strictEqual(status.provider, "codex");

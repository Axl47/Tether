  ...
  switch (method) {
    ...
    case "turn/ended":
      ...
      return {
        ...
        payload: {
          state:
            typeof rawEvent.exitCode === "number" && rawEvent.exitCode !== 0
              ? "failed"
              : "completed",
    ...
    case "gemini/tool_result": {
      const itemId = itemIdFromRaw(rawEvent.tool_id);
      const status =
        rawEvent.status === "failed" || rawEvent.status === "error" ? "failed" : "completed";
      const detail =
        typeof rawEvent.output === "string" && rawEvent.output.trim().length > 0
          ? rawEvent.output
    ...
    case "gemini/error":
      if (rawEvent.severity === "warning") {
        return {
          ...
          payload: {
            message: typeof rawEvent.message === "string" ? rawEvent.message : "Gemini CLI warning",
            detail: rawEvent,
          },
        };
      }
      ...
      respondToUserInput: (_threadId, _requestId, _answers) =>
        Effect.fail(
          new ProviderAdapterRequestError({
            ...
            detail: "Gemini CLI does not support mid-turn user input requests in headless mode.",
          }),
        ) as Effect.Effect<void, ProviderAdapterError>,

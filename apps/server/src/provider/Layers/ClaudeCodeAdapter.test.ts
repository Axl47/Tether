...
function makeHarness(config?: {
  ...
  return {
    ...
    ) as Layer.Layer<ClaudeCodeAdapter, never, never>,
    query,
...
describe("ClaudeCodeAdapterLive", () => {
  ...
  it.effect("does not enable bypass permissions for approval-required sessions", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeCodeAdapter;
      yield* adapter.startSession({
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        const promptMessage = yield* Effect.promise(() =>
          readNextPromptMessage(createInput?.prompt),
        );
  ...
  it.effect("separates distinct Claude text blocks within one turn", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.deepEqual(assistantTextDeltas, ["First sentence.", "\n\n", "Second sentence."]);

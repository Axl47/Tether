  ...
  return Effect.gen(function* () {
    ...
    yield* fs.writeFileString(
      ...
      [
        ...
        "done",
        'stdin_content="$(cat)"',
        'if [ "$TETHER_FAKE_CODEX_REQUIRE_IMAGE" = "1" ] && [ "$seen_image" != "1" ]; then',
        '  printf "%s\\n" "missing --image input" >&2',
        "  exit 2",
        "fi",
        'if [ -n "$TETHER_FAKE_CODEX_STDIN_MUST_CONTAIN" ]; then',
        '  printf "%s" "$stdin_content" | grep -F -- "$TETHER_FAKE_CODEX_STDIN_MUST_CONTAIN" >/dev/null || {',
        '    printf "%s\\n" "stdin missing expected content" >&2',
        "    exit 3",
        "  }",
        "fi",
        'if [ -n "$TETHER_FAKE_CODEX_STDIN_MUST_NOT_CONTAIN" ]; then',
        '  if printf "%s" "$stdin_content" | grep -F -- "$TETHER_FAKE_CODEX_STDIN_MUST_NOT_CONTAIN" >/dev/null; then',
        '    printf "%s\\n" "stdin contained forbidden content" >&2',
        "    exit 4",
        "  fi",
        "fi",
        'if [ -n "$TETHER_FAKE_CODEX_STDERR" ]; then',
        '  printf "%s\\n" "$TETHER_FAKE_CODEX_STDERR" >&2',
        "fi",
        'if [ -n "$output_path" ]; then',
        '  node -e \'const fs=require("node:fs"); const value=process.argv[2] ?? ""; fs.writeFileSync(process.argv[1], Buffer.from(value, "base64"));\' "$output_path" "${TETHER_FAKE_CODEX_OUTPUT_B64:-e30=}"',
        "fi",
        'exit "${TETHER_FAKE_CODEX_EXIT_CODE:-0}"',
        "",
      ].join("\n"),
    );
  ...
  return Effect.acquireUseRelease(
    Effect.gen(function* () {
      ...
      const tempDir = yield* fs.makeTempDirectoryScoped({
        prefix: "tether-codex-text-",
      });
      const binDir = yield* makeFakeCodexBinary(tempDir);
      const previousPath = process.env.PATH;
      const previousOutput = process.env.TETHER_FAKE_CODEX_OUTPUT_B64;
      const previousExitCode = process.env.TETHER_FAKE_CODEX_EXIT_CODE;
      const previousStderr = process.env.TETHER_FAKE_CODEX_STDERR;
      const previousRequireImage = process.env.TETHER_FAKE_CODEX_REQUIRE_IMAGE;
      const previousStdinMustContain =
        process.env.TETHER_FAKE_CODEX_STDIN_MUST_CONTAIN;
      const previousStdinMustNotContain =
        process.env.TETHER_FAKE_CODEX_STDIN_MUST_NOT_CONTAIN;

      yield* Effect.sync(() => {
        process.env.PATH = `${binDir}:${previousPath ?? ""}`;
        process.env.TETHER_FAKE_CODEX_OUTPUT_B64 = Buffer.from(
          input.output,
          "utf8",
        ).toString("base64");

        if (input.exitCode !== undefined) {
          process.env.TETHER_FAKE_CODEX_EXIT_CODE = String(input.exitCode);
        } else {
          delete process.env.TETHER_FAKE_CODEX_EXIT_CODE;
        }

        if (input.stderr !== undefined) {
          process.env.TETHER_FAKE_CODEX_STDERR = input.stderr;
        } else {
          delete process.env.TETHER_FAKE_CODEX_STDERR;
        }

        if (input.requireImage) {
          process.env.TETHER_FAKE_CODEX_REQUIRE_IMAGE = "1";
        } else {
          delete process.env.TETHER_FAKE_CODEX_REQUIRE_IMAGE;
        }
        ...
        if (input.stdinMustContain !== undefined) {
          ...
          delete process.env.TETHER_FAKE_CODEX_STDIN_MUST_CONTAIN;
        }
        ...
        if (input.stdinMustNotContain !== undefined) {
          ...
          delete process.env.TETHER_FAKE_CODEX_STDIN_MUST_NOT_CONTAIN;
        }
      });
    ...
    (previous) =>
      Effect.sync(() => {
        ...
        if (previous.previousOutput === undefined) {
          delete process.env.TETHER_FAKE_CODEX_OUTPUT_B64;
        } else {
          process.env.TETHER_FAKE_CODEX_OUTPUT_B64 = previous.previousOutput;
        }

        if (previous.previousExitCode === undefined) {
          delete process.env.TETHER_FAKE_CODEX_EXIT_CODE;
        } else {
          process.env.TETHER_FAKE_CODEX_EXIT_CODE = previous.previousExitCode;
        }

        if (previous.previousStderr === undefined) {
          delete process.env.TETHER_FAKE_CODEX_STDERR;
        } else {
          process.env.TETHER_FAKE_CODEX_STDERR = previous.previousStderr;
        }

        if (previous.previousRequireImage === undefined) {
          delete process.env.TETHER_FAKE_CODEX_REQUIRE_IMAGE;
        } else {
          process.env.TETHER_FAKE_CODEX_REQUIRE_IMAGE =
  ...
  it.effect("passes image attachments through as codex image inputs", () =>
    withFakeCodexEnv(
      ...
      Effect.gen(function* () {
        ...
        yield* fs.makeDirectory(path.join(process.cwd(), "attachments"), {
          recursive: true,
        });
        ...
        const generated = yield* textGeneration
          ...
          .pipe(
            Effect.ensuring(
              fs.remove(attachmentPath).pipe(Effect.catch(() => Effect.void)),
            ),
          );

...
const CliEnvConfig = Config.all({
  mode: Config.string("TETHER_MODE").pipe(
    Config.option,
    Config.map(
  ...
  port: Config.port("TETHER_PORT").pipe(Config.option, Config.map(Option.getOrUndefined)),
  host: Config.string("TETHER_HOST").pipe(Config.option, Config.map(Option.getOrUndefined)),
  stateDir: Config.string("TETHER_STATE_DIR").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  devUrl: Config.url("VITE_DEV_SERVER_URL").pipe(Config.option, Config.map(Option.getOrUndefined)),
  noBrowser: Config.boolean("TETHER_NO_BROWSER").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  authToken: Config.string("TETHER_AUTH_TOKEN").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  autoBootstrapProjectFromCwd: Config.boolean("TETHER_AUTO_BOOTSTRAP_PROJECT_FROM_CWD").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  logWebSocketEvents: Config.boolean("TETHER_LOG_WS_EVENTS").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
});
...
const makeServerProgram = (input: CliInput) =>
  Effect.gen(function* () {
    ...
    yield* Effect.logInfo("Tether running", {
      ...safeConfig,
...
const stateDirFlag = Flag.string("state-dir").pipe(
  Flag.withDescription("State directory path (equivalent to TETHER_STATE_DIR)."),
  Flag.optional,
);
...
const logWebSocketEventsFlag = Flag.boolean("log-websocket-events").pipe(
  Flag.withDescription(
    "Emit server-side logs for outbound WebSocket push traffic (equivalent to TETHER_LOG_WS_EVENTS).",
  ),
...
export const t3Cli = Command.make("t3", {
  ...
  Command.withDescription("Run the Tether server."),

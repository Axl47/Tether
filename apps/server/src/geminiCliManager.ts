...
export class GeminiCliManager extends EventEmitter {
  ...
  startSession(input: GeminiStartSessionInput): GeminiSessionContext {
    const context: GeminiSessionContext = {
      ...
      hydrating: false,
      ...(input.resumeCursor?.sessionId ? { geminiSessionId: input.resumeCursor.sessionId } : {}),
    };
  ...
  private async runTurnViaAcp(
    ...
    try {
      ...
      const result = await runtime.prompt(context.geminiSessionId, inputPromptBlocks(text, prompt));
      ...
      this.emit("event", {
        ...
        ...(stopReason === "cancelled" ? { error: { message: "Gemini turn cancelled." } } : {}),
        stopReason,
      });
...
async function createGeminiAcpRuntime(
  ...
  child.on("close", (code) => {
    ...
    const message =
      stderrBuffer.trim() || `Gemini ACP runtime exited with code ${code ?? "unknown"}.`;

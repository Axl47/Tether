...
type ThreadActivityRecord = Pick<
  Thread,
...
export function getThreadLatestActivityTimestamp(
  ...
  return Math.max(
    parseIsoTimestamp(thread.updatedAt),
...
export function compareThreadsByLatestActivity(
  ...
  const byLatestActivity =
    getThreadLatestActivityTimestamp(right) -
    getThreadLatestActivityTimestamp(left);
...
export function sortProjectsByLatestThreadUpdate<T extends Pick<Project, "id">>(
  ...
  for (const thread of threads) {
    ...
    const previousTimestamp =
      latestThreadActivityByProjectId.get(thread.projectId) ?? 0;
  ...
  const sortedProjects = projects.map((project, index) => ({
    ...
    latestThreadActivity:
      latestThreadActivityByProjectId.get(project.id) ?? 0,
  }));
  sortedProjects.sort((left, right) => {
    const byLatestThreadActivity =
      right.latestThreadActivity - left.latestThreadActivity;

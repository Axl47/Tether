...
export default function Sidebar() {
  ...
  const isOnDashboard = useLocation({ select: (loc) => loc.pathname === "/dashboard" });
  const isOnSubPage = isOnSettings || isOnDashboard;
  ...
  const [autoRenamingProjectId, setAutoRenamingProjectId] = useState<ProjectId | null>(null);
  ...
  >(() => new Set());
  const [expandedArchivedThreadListsByProject, setExpandedArchivedThreadListsByProject] = useState<
    ReadonlySet<ProjectId>
  >(() => new Set());
  ...
  const serverThreadIdSet = useMemo(() => new Set(threads.map((thread) => thread.id)), [threads]);
  ...
  const draftThreads = useMemo(
    () =>
      (Object.entries(draftThreadsByThreadId) as [ThreadId, DraftThreadState][])
        .flatMap(([threadId, draftThread]) => {
          if (serverThreadIdSet.has(threadId)) {
            return [];
          }
          const fallbackModel = projectModelById.get(draftThread.projectId);
          if (!fallbackModel) {
            return [];
          }
          return [
            buildLocalDraftThread({
              ...
              draftThread,
              fallbackModel,
              composerDraft: draftsByThreadId[threadId],
              error: null,
            }),
          ];
        }),
    [draftThreadsByThreadId, draftsByThreadId, projectModelById, serverThreadIdSet],
  );
  const sidebarThreads = useMemo(() => [...threads, ...draftThreads], [draftThreads, threads]);
  const sidebarThreadById = useMemo(
    () => new Map(sidebarThreads.map((thread) => [thread.id, thread] as const)),
    [sidebarThreads],
  );
  const draftThreadIdSet = useMemo(
    () => new Set(draftThreads.map((thread) => thread.id)),
    [draftThreads],
  );
  ...
  const focusMostRecentThreadForProject = useCallback(
    (projectId: ProjectId) => {
      const latestThread = sidebarThreads
  ...
  const handleAutorenameProjectThreads = useCallback(
    async (projectId: ProjectId, projectName: string) => {
      if (autoRenamingProjectId === projectId) return;

      const api = readNativeApi();
      if (!api) return;

      setAutoRenamingProjectId(projectId);
      try {
        const result = await api.orchestration.autorenameProjectThreads({ projectId });
        ...
        if (renamedCount > 0) {
          toastManager.add({
            ...
            title: renamedCount === 1 ? "Renamed 1 thread" : `Renamed ${renamedCount} threads`,
            description:
              failedCount > 0
                ? `${failedCount} thread${failedCount === 1 ? "" : "s"} failed to rename.`
                : `Updated titles in ${projectName}.`,
          });
          return;
        }
        ...
        if (failedCount > 0) {
          toastManager.add({
            type: "error",
            title: "Could not rename threads",
            description: result.failed[0]?.message ?? "An error occurred.",
          });
          return;
        }
        ...
        toastManager.add({
          type: "info",
          title: "No titles changed",
          description: `No thread titles needed updating in ${projectName}.`,
        });
      } catch (error) {
        toastManager.add({
          type: "error",
        ...
        setAutoRenamingProjectId((current) => (current === projectId ? null : current));
      }
    },
    [autoRenamingProjectId],
  );
  ...
  const handleThreadContextMenuAction = useCallback(
    async (threadId: ThreadId, clicked: string | null) => {
      ...
      if (clicked === "mark-unread" && !isDraftThread) {
      ...
      if (clicked === "archive" && !isDraftThread && thread.archivedAt === null) {
        const shouldNavigateToFallback = routeThreadId === threadId;
        const fallbackThreadId =
          sidebarThreads.find((entry) => entry.id !== threadId && entry.archivedAt === null)?.id ??
          null;
      ...
      if (clicked === "unarchive" && !isDraftThread && thread.archivedAt !== null) {
        await api.orchestration.dispatchCommand({
      ...
      if (appSettings.confirmThreadDelete) {
        const confirmed = await api.dialogs.confirm(
          isDraftThread
            ? [
                `Delete draft "${thread.title}"?`,
                "This removes the unsent draft for this thread.",
              ].join("\n")
            : [
    ...
    async (projectId: ProjectId, clicked: string | null) => {
      ...
      if (projectThreads.length > 0) {
        toastManager.add({
          ...
          description: "Delete all threads and drafts in this project before deleting it.",
        });
        return;
      }
  ...
  const mobileMenuItems: MobileContextMenuItem[] = useMemo(() => {
    ...
    return [];
  }, [mobileMenuThreadId, mobileMenuProjectId, draftThreadIdSet, sidebarThreadById]);
  ...
  const expandArchivedThreadListForProject = useCallback((projectId: ProjectId) => {
    setExpandedArchivedThreadListsByProject((current) => {
      if (current.has(projectId)) return current;
      const next = new Set(current);
      next.add(projectId);
      return next;
  ...
  const collapseArchivedThreadListForProject = useCallback((projectId: ProjectId) => {
    setExpandedArchivedThreadListsByProject((current) => {
      if (!current.has(projectId)) return current;
      const next = new Set(current);
      next.delete(projectId);
  ...
  const renderThreadListItem = (thread: (typeof sidebarThreads)[number]) => {
    ...
    const threadStatus =
      thread.archivedAt === null
        ...
        : null;
    const isDraftThread =
      draftThreadIdSet.has(thread.id) || hasDraftThreadContent(draftsByThreadId[thread.id]);
    const prStatus =
      thread.archivedAt === null ? prStatusIndicator(prByThreadId.get(thread.id) ?? null) : null;
    const terminalStatus =
      thread.archivedAt === null
        ...
        : null;
    const threadTimestamp = threadSort === "activity" ? getThreadLatestActivityAt(thread) : thread.createdAt;
    const threadTimestampTitle = threadSort === "activity" ? "Last activity" : "Created";
      ...
      <SidebarMenuSubItem key={thread.id} className="w-full">
        ...
        >
          <div className="flex min-w-0 flex-1 items-center gap-1.5 text-left">
            ...
            {thread.archivedAt !== null && !isDraftThread && (
            ...
            {isDraftThread && <span className={DRAFT_THREAD_LABEL_CLASS_NAME}>Draft</span>}
        ...
        <SidebarGroup className="px-2 py-2">
          ...
          <SidebarMenu>
            {projects.map((project) => {
              ...
              const activeProjectThreads = sortSidebarThreads(
                filteredProjectThreads.filter((thread) => thread.archivedAt === null),
                {
                  sortBy: threadSort,
              ...
              const archivedProjectThreads = sortSidebarThreads(
                filteredProjectThreads.filter((thread) => thread.archivedAt !== null),
                {
                  sortBy: threadSort,
                  hasPendingApprovalsByThreadId: pendingApprovalByThreadId,
              ...
              const hasHiddenThreads =
                !hasActiveThreadSearch && activeProjectThreads.length > THREAD_PREVIEW_LIMIT;
              const isAutoRenamingProject = autoRenamingProjectId === project.id;
              const visibleThreads =
                hasHiddenThreads && !isThreadListExpanded
                  ? activeProjectThreads.slice(0, THREAD_PREVIEW_LIMIT)
                  : activeProjectThreads;
              const hasArchivedMatches = archivedProjectThreads.length > 0;
              const isArchivedSectionExpanded = hasActiveThreadSearch
                ? hasArchivedMatches
                : expandedArchivedThreadListsByProject.has(project.id);
                  ...
                  <SidebarMenuItem>
                          ...
                          <TooltipTrigger
                            render={
                              <button
                                ...
                                disabled={
                                  isAutoRenamingProject || activeProjectThreads.length === 0
                                }
                                onClick={(event) => {
                                  event.preventDefault();
                                  event.stopPropagation();
                                  void handleAutorenameProjectThreads(project.id, project.name);
                                }}
                              >
                                <SparklesIcon
                                  className={`size-3.5 ${isAutoRenamingProject ? "animate-pulse" : ""}`}
                                />
                              </button>
                            }
                          />
                          <TooltipPopup side="top">
                            {isAutoRenamingProject
                              ? "Renaming thread titles..."
                              : "Magic rename thread titles"}
                          </TooltipPopup>
                        </Tooltip>
                        <Tooltip>
                          <TooltipTrigger
                    ...
                    <CollapsibleContent>
                      <SidebarMenuSub className="mx-1 my-0 translate-x-0 gap-0 px-1.5 py-0">
                        ...
                        {visibleThreads.map(renderThreadListItem)}
                        ...
                        {hasArchivedMatches && (
                          <>
                              ...
                              <SidebarMenuSubButton
                                ...
                                onClick={() => {
                                  if (isArchivedSectionExpanded) {
                                    collapseArchivedThreadListForProject(project.id);
                                  } else {
                                    expandArchivedThreadListForProject(project.id);
                                  }
                                }}
                              >
                                <ChevronRightIcon
                                  className={`size-3 shrink-0 transition-transform duration-150 ${
                                    isArchivedSectionExpanded ? "rotate-90" : ""
                                  }`}
                                />
                            ...
                            {isArchivedSectionExpanded
                              ? archivedProjectThreads.map(renderThreadListItem)
                              : null}
                          </>
                        )}

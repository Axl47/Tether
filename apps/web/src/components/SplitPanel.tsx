...
function ResizeHandle({
  ...
  return (
    <div
      ...
      className={direction === "horizontal" ? "w-2 cursor-col-resize" : "h-2 cursor-row-resize"}
    />
  );
}
...
export function SplitDropPreview({
  ...
  return (
    <div className="relative flex h-full min-h-0 w-full min-w-0 flex-1 overflow-hidden rounded-[inherit]">
      {children}
      {zone === "center" ? (
  ...
  if (node.type === "leaf") {
    ...
    return (
      <LeafPane
        ...
        isZoomed={zoomedLeafId === null || node.id === zoomedLeafId}
  ...
  return (
    <div className="flex h-full min-h-0 min-w-0 flex-1 overflow-hidden rounded-lg bg-muted p-1">
      <SplitPanelNode
        ...
        showDropZones={!zoomed}
        renderLeaf={renderLeaf}
        onSplitDrop={onSplitDrop}
        onFocusLeaf={onFocusLeaf}
        zoomedLeafId={zoomedLeaf?.id ?? null}
      />
    </div>
  );
}

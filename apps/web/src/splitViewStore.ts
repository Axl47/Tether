...
export type PersistedBrowserPaneState = Pick<
  BrowserPaneLeaf,
  "paneId" | "url" | "targetThreadId" | "createdFromThreadId"
>;
...
export function findBrowserLeafByPaneId(node: SplitNode, paneId: string): BrowserPaneLeaf | null {
  ...
  return (
    findBrowserLeafByPaneId(node.children[0], paneId) ??
    findBrowserLeafByPaneId(node.children[1], paneId)
  );
}
...
function updateBrowserLeafState(
  node: SplitNode,
  paneId: string,
  state: Partial<PersistedBrowserPaneState>,
): SplitNode | null {
  if (node.type === "leaf") {
...
function isValidLeaf(leaf: unknown): leaf is SplitLeaf {
  ...
  if (c.paneType === "browser") {
    return (
      typeof c.paneId === "string" &&
      typeof c.url === "string" &&
      typeof c.targetThreadId === "string" &&
      typeof c.createdFromThreadId === "string"
    );
  }
  return false;
}
...
function isValidNode(node: unknown): node is SplitNode {
  ...
  return (
    ...
    c.children.length === 2 &&
    isValidNode(c.children[0]) &&
    isValidNode(c.children[1])
  );
}
...
function sanitizeWorkspace(workspace: Workspace): Workspace | null {
  const pruned = pruneInvalidLeaves(
    workspace.root,
    new Set(collectThreadIds(workspace.root)),
    isDesktopMode(),
  );
...
function readPersistedSplitView(): PersistedSplitViewState {
  ...
  const parseModern = (key: string): PersistedSplitViewState | null => {
    ...
    return {
      workspaces,
      activeWorkspaceId: resolveActiveWorkspace(workspaces, parsed.activeWorkspaceId)?.id ?? null,
    };
  };
  ...
  try {
    return (
      parseModern(SPLIT_VIEW_STORAGE_KEY) ??
      parseModern(LEGACY_SPLIT_VIEW_STORAGE_KEY) ??
      parseLegacySingle() ?? { workspaces: [], activeWorkspaceId: null }
    );
  } catch {
...
export interface SplitViewActions {
  ...
  updateBrowserPanePersistedState: (
    paneId: string,
    state: Partial<PersistedBrowserPaneState>,
  ) => void;
...
export const useSplitViewStore = create<SplitViewStore>((set, get) => ({
  ...
  splitThread: (currentThreadId, newThreadId, direction, insertBefore) => {
    set((state) => {
      ...
      if (workspace) {
        ...
        if (existingLeaf) {
          ...
          return {
            ...withWorkspaceCollection(
              updateWorkspace(state.workspaces, workspace.id, () => updated),
              workspace.id,
            ),
            zoomed: false,
            dragOver: null,
        ...
        const updated = touchWorkspace({
          ...workspace,
          root: newRoot,
          focusedLeafId: inserted?.id ?? workspace.focusedLeafId,
        });
        return {
          ...withWorkspaceCollection(
            updateWorkspace(state.workspaces, workspace.id, () => updated),
      ...
      const root: SplitBranch = {
        ...
        id: splitNodeId(),
        direction,
        children: insertBefore ? [newLeaf, existingLeaf] : [existingLeaf, newLeaf],
        ratio: 0.5,
      };
      const ws: Workspace = {
        id: root.id,
  ...
  splitLeaf: (leafId, newThreadId, direction, insertBefore) => {
    set((state) => {
      ...
      if (!targetLeaf || (targetLeaf.paneType === "thread" && targetLeaf.threadId === newThreadId))
        return state;
      ...
      if (existingLeaf) {
        ...
        return {
          ...withWorkspaceCollection(
            updateWorkspace(state.workspaces, state.group.id, () => updated),
            state.group.id,
          ),
          zoomed: false,
          dragOver: null,
      ...
      const updated = touchWorkspace({
        ...state.group,
        root: newRoot,
        focusedLeafId: inserted?.id ?? state.group.focusedLeafId,
      });
      return {
        ...withWorkspaceCollection(
          updateWorkspace(state.workspaces, state.group.id, () => updated),
  ...
  splitThreadWithBrowser: (currentThreadId, direction) => {
    ...
    set((state) => {
      const browserLeaf = createBrowserLeaf({
        paneId,
        url: "about:blank",
        targetThreadId: currentThreadId,
        createdFromThreadId: currentThreadId,
      });
      const workspace = findWorkspaceContainingThread(state.workspaces, currentThreadId);
      if (workspace) {
        const group = state.workspaces.find((w) => w.id === workspace.id) ?? workspace;
        const currentLeaf = findLeafByThreadId(group.root, currentThreadId);
        if (!currentLeaf) return state;
        const root = splitLeafNode(group.root, currentLeaf.id, browserLeaf, direction, false);
        if (!root) return state;
        const insertedLeaf = findBrowserLeafByPaneId(root, paneId);
        const updated = touchWorkspace({
          ...group,
          root,
          focusedLeafId: insertedLeaf?.id ?? group.focusedLeafId,
        });
        return {
          ...withWorkspaceCollection(
            updateWorkspace(state.workspaces, group.id, () => updated),
            group.id,
          ),
          zoomed: false,
          dragOver: null,
      ...
      const currentLeaf = createThreadLeaf(currentThreadId);
      const root: SplitBranch = {
        type: "branch",
        id: splitNodeId(),
        direction,
        children: [currentLeaf, browserLeaf],
        ratio: 0.5,
      };
      const workspaceId = root.id;
      const workspaceToCreate: Workspace = {
        id: workspaceId,
        name: buildNextWorkspaceName(state.workspaces),
        lastVisitedAt: new Date().toISOString(),
        root,
        focusedLeafId: browserLeaf.id,
      };
      return {
        ...withWorkspaceCollection([...state.workspaces, workspaceToCreate], workspaceId),
        zoomed: false,
        dragOver: null,
      };
    });
  ...
  closePane: (leafId) => {
    ...
    if (!remaining || remaining.type === "leaf") {
      ...
      set({
        ...withWorkspaceCollection(
          state.workspaces.filter((w) => w.id !== state.group?.id),
          null,
        ),
        zoomed: false,
        dragOver: null,
    ...
    const focusedLeaf =
      state.group.focusedLeafId === leafId
        ? firstLeaf(remaining)
        : (findLeaf(remaining, state.group.focusedLeafId) ?? firstLeaf(remaining));
    const updated = touchWorkspace({
      ...state.group,
      root: remaining,
  ...
  renameWorkspace: (workspaceId, name) => {
    ...
    set((state) => ({
      ...withWorkspaceCollection(
        updateWorkspace(state.workspaces, workspaceId, (workspace) =>
          workspace.name === trimmed ? workspace : { ...workspace, name: trimmed },
        ),
  ...
  deactivateWorkspace: () =>
    set((state) =>
      state.activeWorkspaceId === null && state.group === null
        ? state
        : { ...withWorkspaceCollection(state.workspaces, null), zoomed: false, dragOver: null },
    ),
  setFocusedLeaf: (leafId) =>
    set((state) =>
      !state.group || state.group.focusedLeafId === leafId
        ? state
        : {
            ...withWorkspaceCollection(
              updateWorkspace(state.workspaces, state.group.id, () =>
                touchWorkspace({ ...state.group!, focusedLeafId: leafId }),
              ),
              state.group.id,
            ),
          },
  ...
  unsplit: () => {
    ...
    set({
      ...withWorkspaceCollection(
        state.workspaces.filter((workspace) => workspace.id !== state.group?.id),
        null,
      ),
      zoomed: false,
      dragOver: null,
  ...
  focusDirection: (direction) => {
    ...
    set({
      ...withWorkspaceCollection(
        updateWorkspace(state.workspaces, state.group.id, () =>
          touchWorkspace({ ...state.group!, focusedLeafId: target.id }),
        ),
        state.group.id,
      ),
      zoomed: false,
  ...
  reconcileThreads: (validThreadIds) => {
    ...
    set({
      ...withWorkspaceCollection(nextWorkspaces, nextActive?.id ?? null),
      zoomed: false,
      dragOver: null,
    });

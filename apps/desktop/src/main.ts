...
import { createBrowserPaneManager } from "./browserPaneManager.ts";
...
const STATE_DIR = process.env.TETHER_STATE_DIR?.trim() || Path.join(BASE_DIR, "userdata");
...
const APP_USER_MODEL_ID = isDevelopment ? "com.axorio.tether.dev" : "com.axorio.tether";
const LINUX_DESKTOP_ENTRY_NAME = isDevelopment ? "tether-dev.desktop" : "tether.desktop";
const LINUX_WM_CLASS = isDevelopment ? "tether-dev" : "tether";
const USER_DATA_DIR_NAME = isDevelopment ? "tether-dev" : "tether";
const LEGACY_USER_DATA_DIR_NAMES = [
  desktopAppBranding.displayName,
  isDevelopment ? "Tether (Dev)" : "Tether (Alpha)",
...
function backendChildEnv(): NodeJS.ProcessEnv {
  ...
  delete env.TETHER_PORT;
  delete env.TETHER_MODE;
  delete env.TETHER_NO_BROWSER;
  delete env.TETHER_HOST;
  delete env.TETHER_STATE_DIR;
  delete env.TETHER_AUTH_TOKEN;
  delete env.TETHER_DESKTOP_WS_URL;
  return env;
}
...
function ensureInitialBackendWindowOpen(): void {
  ...
  const nextOpen = waitForBackendWindowReady(backendHttpUrl)
    .then((source) => {
      ...
      if (mainWindow ?? BaseWindow.getAllWindows()[0]) {
        return;
      }
...
function resolveEmbeddedCommitHash(): string | null {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { tetherCommitHash?: unknown };
    return normalizeCommitHash(parsed.tetherCommitHash);
  } catch {
    return null;
  }
}
...
function resolveAboutCommitHash(): string | null {
  ...
  const envCommitHash = normalizeCommitHash(process.env.TETHER_COMMIT_HASH);
  if (envCommitHash) {
...
function handleCheckForUpdatesMenuClick(): void {
  ...
  const disabledReason = getAutoUpdateDisabledReason({
    ...
    disabledByEnv:
      process.env.TETHER_DISABLE_AUTO_UPDATE === "1" ||
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `Tether ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
 ...
 * parentheses (e.g. `~/.config/Tether (Alpha)` on Linux). This is
 * unfriendly for shell usage and violates Linux naming conventions.
 *
 * We override it to a clean lowercase name (`tether`). If an older
 * Tether or T3 Code directory already exists we keep using it so
 * existing users do not lose Chromium profile data (localStorage,
 * cookies, sessions).
 */
...
function revealWindow(window: BaseWindow): void {
...
function syncWindowAppearance(window: BaseWindow): void {
  if (window.isDestroyed()) {
    return;
  }
...
function createWindow(): BaseWindow {
  ...
  appView.webContents.on("context-menu", (event, params) => {
    ...
    if (params.mediaType === "image") {
      menuTemplate.push({
        label: "Copy Image",
        click: () => appView.webContents.copyImageAt(params.x, params.y),
      });
      menuTemplate.push({ type: "separator" });
    }
  ...
  if (isDevelopment) {
    void appView.webContents.loadURL(resolveDesktopDevServerUrl());
    appView.webContents.openDevTools({ mode: "detach" });
  } else {
    void appView.webContents.loadURL(backendHttpUrl);
  }
